  ...
  try {
    const result = await getPool().query(
      ...
      SELECT
        ...
        COUNT(DISTINCT filings.id) AS filing_count,
        COUNT(documents.id) AS document_count
      FROM customers
      LEFT JOIN filings ON customers.id = filings.customer_id
      LEFT JOIN documents ON filings.id = documents.filing_id
      GROUP BY customers.id, customers.name, customers.email, customers.phone, customers.pan_number
...
// ==========================================
export const getCustomerFilings = async (req, res) => {
  try {
  ...
  try {
    ...
    const signedDocuments = await Promise.all(
      documentsResult.rows.map(async (doc) => {
        // Safely handle both snake_case (DB) and camelCase (runtime) property names
        ...
        if (currentUrl) {
          try {
            if (currentUrl.includes("digitaloceanspaces.com")) {
              ...
              const command = new GetObjectCommand({    // ✅ Now properly imported
                Bucket: "dtr-file-storage",
                Key: fileKey,
              });

              // Secure presigned URL valid for 1 hour
              const presignedUrl = await getSignedUrl(s3Client, command, { expiresIn: 3600 }); // ✅ Now properly imported
        ...
        return doc; // Pass through legacy Cloudinary URLs untouched
      })
    );
...
export const deleteDocument = async (req, res) => {
  try {
    ...
    await getPool().query(`DELETE FROM documents WHERE id = $1`, [documentId]);
